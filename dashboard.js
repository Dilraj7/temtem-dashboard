fetch("data.json")
  ...
  .then((data) => {
    ...
    const avgLuma = (rows.reduce((sum, t) => sum + (t.lumaChance || 0), 0) / rows.length) * 100;
    document.getElementById("avgLuma").textContent = avgLuma.toFixed(2) + " %";

    // Donut Chart: Top 5 LumaChance
    const top5 = [...rows]
    ...
    // 💡 Barre de recherche (placée ici hors boucle)
    ...
    rows.forEach((t) => {
      ...
      flipCard.addEventListener("click", async function () {
        ...
        const back = flipCard.querySelector(".temtem-flip-back");
